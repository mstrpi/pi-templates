...
echo -e "${clr}To confirm, are you running a Debian/Ubuntu derivitive?${nrm} "
...
echo -e "${clr}Confirmation of WireGuard Interface:${nrm} "
sudo wg
sleep 3
echo -e "${clr}What is the name of the 1st peer/device to be added to this VPN setup?${nrm} "
read -r client
echo -e "${clr}Creating config for ${client}.${nrm} "
mkdir -p /etc/wireguard/${client}/
echo -e "${clr}One moment, please...${nrm} "
sleep 2
...
{ echo "[Peer]" ; echo "PublicKey = $(cat /etc/wireguard/${client}/${client}.pub)" ; echo "PresharedKey = $(cat /etc/wireguard/${client}/${client}.psk)" ; echo -e "AllowedIPs = 10.100.0.2/32, fd08:4711::2/128" ; echo ; } >> /etc/wireguard/wg0.conf
...
echo -e "${clr}Installing QRencode. This will grant the ability to scan a QR code for easily importing this config file to ${client}.${nrm} "
sleep 4
if [ "${debian}" = "y" ]
then sudo apt install -y qrencode
else sudo yum install -y qrencode
fi
echo -e "${clr}Please enter the external IP address, or the domain name for this connection:${nrm} "
read -r conn
echo -e "${clr}Great! Almost there!${nrm} "
echo -e "${clr}Enabling forwarding:${nrm} "
...
echo -e "${clr}Generating QR code:${nrm} "
sleep 2
